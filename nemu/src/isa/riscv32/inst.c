...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, if(rd == 0) rd = 1; R(rd) = s->snpc; s->dnpc = s->pc+imm;);
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, if(rd == 0) rd = 1; R(rd) = s->snpc; s->dnpc = (src1+imm) & (~1););
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra    , R, R(rd) = src1 >> BITS(src2, 4, 0));
  INSTPAT("010000 ?????? ????? 101 ????? 00100 11", srai   , I, if((imm - 1024) < 32) R(rd) = ((sword_t)src1 >> (imm - 1024)));
